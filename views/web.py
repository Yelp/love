...
def love_link(link_id):
    try:
        ...
        loved = [
            Employee.get_key_for_username(recipient).get() for recipient in recipients
        ]
...
def explore():
    ...
    if not username:
        ...
        return render_template(
            "explore.html", current_time=datetime.utcnow(), user=None
        )
    username = username.lower().strip()

    user_key = Employee.query(
        Employee.username == username, Employee.terminated == False,  # noqa
    ).get(keys_only=True)
    ...
    received_love = logic.love.recent_received_love(
        user_key, include_secret=False, limit=20
    )
...
def sent():
    ...
    recipients = sanitize_recipients(recipients_str)
    loved = [Employee.get_key_for_username(recipient).get() for recipient in recipients]
...
def keys():
    ...
    return render_template("keys.html", keys=api_keys)
...
def create_key():
    ...
    flash(
        "Your API key {} has been created. Refresh the page to see it below.".format(
            new_key.access_key
        )
    )
...
def love():
    ...
    try:
        if action == "create_link":
            ...
            if secret:
                ...
                hash_key = (
                    link_id
                    if link_id
                    else create_love_link(real_display_str, message).hash_key
                )
                return redirect(
                    url_for(
                        "sent",
        ...
        return redirect(
            url_for("home", recipients=recipients_display_str, message=message)
        )
...
@admin_required
def aliases():
    return render_template("aliases.html", aliases=Alias.query().fetch(),)
...
def create_alias():
    try:
        logic.alias.save_alias(
            request.form.get("alias").strip(), request.form.get("username").strip(),
        )
...
def import_employees_form():
    ...
    return render_template("import.html", import_file_exists=import_file_exists,)
