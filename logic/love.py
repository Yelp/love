...
def recent_sent_love_page(employee_key, start_dt=None, end_dt=None, include_secret=True, limit=20, page=1):
    ...
    offset = limit * (page - 1)
    return query.fetch_page_async(limit, offset=offset)


def recent_received_love_page(employee_key, start_dt=None, end_dt=None, include_secret=True, limit=20, page=1):
    """ Returns results, next_cursor, more for paginated love requests """
    query = _received_love_query(employee_key, start_dt, end_dt, include_secret)
    if not limit:
        # Just use our default
